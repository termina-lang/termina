...
renderExpression expr = 
  ...
  case runState (runExceptT (genExpression expr)) (mkCGeneratorEnv "test" S.empty emptyMonadicTypes config M.empty) of
...
renderStatement stmt = 
  case runExcept (genBBlocks [] [stmt]) of
    ...
    Right bBlocks ->
      ...
      case runState (runExceptT (Prelude.concat <$> traverse genBlocks bBlocks)) (mkCGeneratorEnv "test" S.empty emptyMonadicTypes config irqMap) of
        (Left err, _) -> pack $ show err
...
renderTypeDefinitionDecl monTypes decl = 
  case runExcept . genBBAnnASTElement $ decl of
    ...
    Right bbDecl ->
      let config = defaultConfig "test" TestPlatform
          irqMap = getPlatformInterruptMap TestPlatform in
      case runState (runExceptT (genTypeDefinitionDecl bbDecl)) (mkCGeneratorEnv "test" S.empty monTypes config irqMap) of
        (Left err, _) -> pack $ show err
...
renderFunctionDecl monTypes decl = 
  case runExcept . genBBAnnASTElement $ decl of
    ...
    Right bbAST -> 
      let config = defaultConfig "test" TestPlatform
          irqMap = getPlatformInterruptMap TestPlatform in
      case runState (runExceptT (genFunctionDecl bbAST)) (mkCGeneratorEnv "test" S.empty monTypes config irqMap) of
        (Left err, _) -> pack $ show err
        (Right cDecls, _) -> render $ vsep $ runReader (mapM pprint cDecls) (CPrinterConfig False False) 
...
renderFunction func = 
  case runExcept . genBBAnnASTElement $ func of
    ...
    Right bbAST -> 
      let config = defaultConfig "test" TestPlatform
          irqMap = getPlatformInterruptMap TestPlatform in
      case runState (runExceptT (genFunction bbAST)) (mkCGeneratorEnv "test" S.empty emptyMonadicTypes config irqMap) of
        (Left err, _) -> pack $ show err
        (Right cDecls, _) -> render $ vsep $ runReader (mapM pprint cDecls) (CPrinterConfig False False)