...
-- CFE-017: an unsigned expression is never less than zero.
...
-- CFE-017: with the constant on the left, 255 >= x holds for every u8.
...
spec = describe "ConstFolding: error detail (carried value)" $ do
  it "CFE-017 carries the constant and the fixed result (lower limit)" $
    constFoldError unsignedBelowZero `shouldSatisfy` \case
      Just (AnnotatedError (EInvariantComparison 0 _ False) _) -> True
      _ -> False
  it "CFE-017 carries the constant and the fixed result (upper limit, swapped operands)" $
