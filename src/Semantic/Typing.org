...
#+begin_src haskell
typeExpression (MatchExpression e cs) =
  -- | Pattern Matching.
  {-
\[
\begin{array}{c}
  ...
  where
    ...
    pmOption' ty csnone cssome =
     ...
     then if someConditions cssome
          then do
            tyNone <- typeBlockRet (matchBody csnone)
            tySome <- case matchBVars cssome of
              [v] -> addTempVars [(v,ty)] (typeBlockRet (matchBody cssome))
