...
data Error
  ...
  | EInvalidProcedureCallInsideMemberFunction Identifier -- ^ Invalid procedure call inside member function (SE-128)
  ...
  | EAccessPortFieldInvalidType TerminaType -- ^ Invalid access port field type (SE-134)
  | EAccessPortConnectionInvalidGlobal Identifier -- ^ Invalid access port connection (SE-135)
  ...
  | EStructDefEmpty Identifier -- ^ Empty struct definition (SE-143)
  | EEnumDefEmpty Identifier -- ^ Empty enum definition (SE-144)
  | EEnumDefNotUniqueVariant [Identifier] -- ^ Repeated variant in enum definition (SE-145)
  | EInterfaceEmpty Identifier -- ^ Empty interface definition (SE-146)
  | EInterfaceNotUniqueProcedure [Identifier] -- ^ Repeated procedure in interface definition (SE-147)
  | EClassLoop [Identifier] -- ^ Loop between member function calls in class definition (SE-148)
  | EDereferenceInvalidType TerminaType -- ^ Invalid dereference type (SE-149)
  | EMatchInvalidType TerminaType -- ^ Invalid match type (SE-150)
  | EMatchCaseUnknownVariant Identifier -- ^ Unknown variant in match case (SE-151)
  | EMatchMissingCases [Identifier] -- ^ Missing cases in match statement (SE-152)
  | EMatchExtraCases [Identifier] -- ^ Extra cases in match statement (SE-153)
  | EIsVariantInvalidType TerminaType -- ^ Invalid type for is variant expression (SE-154)
  | EIsOptionVariantInvalidType TerminaType -- ^ Invalid type for is option variant expression (SE-155)
  | EIsVariantEnumTypeMismatch Identifier Identifier -- ^ Enum type mismatch in is variant expression (SE-156)
  | EOutputPortInvalidProcedure Identifier -- ^ Invalid procedure in output port (SE-157)
  | EInvalidPoolInitialization -- ^ Invalid pool initialization (SE-158)
  | EInvalidAtomicInitialization -- ^ Invalid atomic initialization (SE-159)
  | EInvalidAtomicArrayInitialization -- ^ Invalid atomic array initialization (SE-160)
  | EInvalidMsgQueueInitialization -- ^ Invalid message queue initialization (SE-161)
  | EUnknownGlobal Identifier -- ^ Unknown global object (SE-162)
  deriving Show
