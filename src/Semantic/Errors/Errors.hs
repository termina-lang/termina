...
data Error
  ...
  | EFieldValueAssignmentMissingFields (Identifier, Location) [Identifier] -- ^ Missing field/s in field assignment expression (SE-119)
  | EFieldValueAssignmentExtraFields (Identifier, Location) [Identifier] -- ^ Extra field/s in field assignment expression (SE-120)
  ...
  | ESinkPortConnectionInvalidGlobal Identifier -- ^ Invalid sink port connection (SE-135)
  | EInboundPortConnectionInvalidObject Identifier -- ^ Invalid inbound port connection (SE-136)
  | EOutboundPortConnectionInvalidGlobal Identifier -- ^ Invalid outbound port connection (SE-137)
  | EAllocatorPortConnectionInvalidGlobal Identifier -- ^ Invalid allocator port connection (SE-138)
  | EAtomicAccessPortConnectionInvalidGlobal Identifier -- ^ Invalid atomic access port connection (SE-138)
  | EAtomicArrayAccessPortConnectionInvalidGlobal Identifier -- ^ Invalid atomic array access port connection (SE-140)
  | EStructDefNotUniqueField [Identifier] -- ^ Repeated field in struct definition (SE-141)
  | EStructDefEmpty Identifier -- ^ Empty struct definition (SE-142)
  | EEnumDefEmpty Identifier -- ^ Empty enum definition (SE-143)
  | EEnumDefNotUniqueVariant [Identifier] -- ^ Repeated variant in enum definition (SE-144)
  | EInterfaceEmpty Identifier -- ^ Empty interface definition (SE-145)
  | EInterfaceNotUniqueProcedure [Identifier] -- ^ Repeated procedure in interface definition (SE-146)
  | EClassLoop [Identifier] -- ^ Loop between member function calls in class definition (SE-147)
  | EDereferenceInvalidType TerminaType -- ^ Invalid dereference type (SE-148)
  | EMatchInvalidType TerminaType -- ^ Invalid match type (SE-149)
  | EMatchCaseUnknownVariant Identifier -- ^ Unknown variant in match case (SE-150)
  | EMatchMissingCases [Identifier] -- ^ Missing cases in match statement (SE-151)
  | EMatchExtraCases [Identifier] -- ^ Extra cases in match statement (SE-152)
  | EIsVariantInvalidType TerminaType -- ^ Invalid type for is variant expression (SE-153)
  | EIsOptionVariantInvalidType TerminaType -- ^ Invalid type for is option variant expression (SE-154)
  | EIsVariantEnumTypeMismatch Identifier Identifier -- ^ Enum type mismatch in is variant expression (SE-155)
  | EOutputPortInvalidProcedure Identifier -- ^ Invalid procedure in output port (SE-156)
  | EInvalidPoolInitialization -- ^ Invalid pool initialization (SE-157)
  | EInvalidAtomicInitialization -- ^ Invalid atomic initialization (SE-158)
  | EInvalidAtomicArrayInitialization -- ^ Invalid atomic array initialization (SE-159)
  | EInvalidMsgQueueInitialization -- ^ Invalid message queue initialization (SE-160)
  | EUnknownGlobal Identifier -- ^ Unknown global object (SE-161)
  deriving Show
