...
genBlock currentCmp acc (TPBlockMemberFunctionCall args called _pos _) = do
    stepStmts <- genActivity currentCmp called args
...
genActivity :: Identifier -> TransPathActivity a -> [ConstExpression a] -> PlantUMLGenMonad [PlantUMLSeqDiagramStatement]
genActivity prevCmp (TRPTaskActivity stepName taskName actionName pathName blocks nextSteps _ _) _ = do
    nextActivities <- mapM (\actId -> do
        ...
        case M.lookup actId stMap of
            ...
            Just act -> genActivity taskName act []
        ) nextSteps
...
genActivity prevCmp (TRPHandlerActivity stepName handlerName actionName pathName blocks nextSteps _ _) _ = do
    nextActivities <- mapM (\actId -> do
        stMap <- ST.gets stepMap
        case M.lookup actId stMap of
            Nothing -> throwError $ T.pack $ "Activity not found in step map: " ++ actId
            Just act -> genActivity handlerName act []
        ) nextSteps
    let activityName = T.pack "<b>" <> T.pack stepName <> "</b> " <> T.pack actionName <> "::" <> T.pack pathName
    ...
    return $ startMsg : stepStmts ++ concat nextActivities
genActivity prevCmp (TRPResourceActivity currentCmp procName pathName blocks _ _) args = do
    ...
    case M.lookup initialStep stMap of
        ...
        Just initialAct -> 
            let initialState = PlantUMLGenState stMap M.empty
                (result, finalState) = ST.runState (runExceptT (genActivity emitterId initialAct [])) initialState
            in case result of
                Left err -> Left err
                Right stmts -> 
